...
import { readBrandPack, readEnvelope } from './content';
 ...
 * reply — and nothing else. It deliberately does not restate the envelope or the
 * packs: those are the signed-off wording and they are loaded verbatim.
 */

const OPERATING_INSTRUCTION = `You are the assistant inside Sadie's marketing cover app.
...
export async function buildSystemPrompt(context: Context): Promise<Anthropic.TextBlockParam[]> {
  const [envelope, pack] = await Promise.all([readEnvelope(), readBrandPack(context)]);

  return [
    { type: 'text', text: OPERATING_INSTRUCTION },
    { type: 'text', text: `--- Authority envelope ---\n\n${envelope}` },
    {
      ...
      // conversation, so it is worth caching. The date below changes daily and
      // sits after the breakpoint so it never invalidates the rest.
      cache_control: { type: 'ephemeral' },
    },
    { type: 'text', text: `Today is ${todayInLondon()}.` },
  ];
}
